...
bool TimeStamp::operator<(const TimeStamp &rhs) const { return time_ < rhs.GetTime(); }
bool TimeStamp::operator<=(const TimeStamp &rhs) const { return time_ <= rhs.GetTime(); }
bool TimeStamp::operator==(const TimeStamp &rhs) const { return time_ == rhs.GetTime(); }
...
std::string TimeStamp::ToFormattedString() const {
  ...
  snprintf(buf, sizeof(buf), "%4d-%02d-%02d %02d:%02d:%02d.%06ld", tm_time.tm_year + 1900, tm_time.tm_mon + 1,
           tm_time.tm_mday, tm_time.tm_hour, tm_time.tm_min, tm_time.tm_sec, micro_seconds);
  return buf;
}

time_t TimeStamp::GetTime() const { return time_; }

void TimeStamp::SetTime(time_t time) { time_ = time; }
